...
export async function GET(req: Request) {
  try {
    ...
    const approved = store.matches.filter((m) => m.candidateId === userId && m.status === "approved");
    const jobs = approved.map((m) => {
      ...
      return { matchId: m.id, score: m.score, reason: m.reason, employerName: user?.name ?? "מעסיק/ה", card: er?.card };
    });
    return ok({ jobs });
  } catch (e) {
    return fail(e);
