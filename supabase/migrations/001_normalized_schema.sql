-- Normalized schema for shidukh-poc (FR-DATA)
-- Cards use JSONB so new fields can grow at runtime via `extras` without DDL changes.
...
-- Runtime field registry: employers / AI can register new card keys without migrations.
...
-- Legacy blob kept for one-time import from older deployments.
